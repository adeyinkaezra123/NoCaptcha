...
function assets() {
  return src('src/wiki/assets/**/*.svg', {base: '.'})
    .pipe(dest(distDir));
}
...
function license() {
  ...
  const notice = `NoCaptcha: CAPTCHA Solver for Busy People
Copyright (c) ${year} Ezra Adeyinka
...
function secrets(done) {
  try {
    let data = process.env.NOCAPTCHA_SECRETS;
    if (data) {
...
exports.build = series(
  init,
  parallel(js, html, css, images, assets, fonts, locale, manifest, license),
  secrets
);
exports.zip = zip;
